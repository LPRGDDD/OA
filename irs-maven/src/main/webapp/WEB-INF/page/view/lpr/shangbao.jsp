...
<body>
           ...
           <!-- 员工薪酬基数设置-->
               ...
               <!--  查询员工ID、人员名称 -->
                  <div class="layui-form-item" style="margin-bottom:1px;margin-top:1px;" align="center">
			          ...
			          <div class="layui-input-inline">
			         	<input type="hidden" id="adminids"/><!-- 员工薪酬ID -->
			         	<input type="hidden" value="${salaryflow_id}" name="salaryflow.salaryflowId"/>
			    ...
			    <!-- 保险项 -->
			       <div class="layui-form-item" style="margin-bottom:1px;margin-top:1px;" align="center">
			         <label class="layui-form-label">保险基数</label>
			          <div class="layui-input-inline">
			            <input name="personxcInsurejs" id="zong" onblur="jisuan()"  lay-verify="required" placeholder="请输入金额" autocomplete="off" class="layui-input" type="text"/>
			       ...
			       <div class="layui-form-item" style="margin-bottom:1px;margin-top:1px;">
			        ...
			        <div class="layui-form-item" style="margin-bottom:1px;margin-top:1px;">
			          <label class="layui-form-label">实发工资</label>
			            <div class="layui-input-inline">
			         	   <input name="salSsalary" id="sfgz" lay-verify="required" placeholder="请输入金额" autocomplete="off" class="layui-input" type="text"/>
			            </div>
			        </div>
			        <div class="layui-form-item" style="margin-bottom:1px;margin-top:1px;">
			          <label class="layui-form-label">员工表现</label>
			            <div class="layui-input-inline">
			         	   <input name="personxcExpression" id="ygbx" lay-verify="required" placeholder="请输入金额" autocomplete="off" class="layui-input" type="text"/>
			            </div>
			        </div>
	...
	<!-- 查询所有的员工 -->
	<div class="layui-row" id="kuangAll" style="display:none;">
		<table class="layui-hide" id="userAll" lay-filter="userAll"></table>
		<div id="userAll"></div>
		<div class="layui-form-item">
			<div calss="layui-input-block">
				<button class="layui-btn" id="userAllbtn" >立即提交</button>
				<button type="reset" class="layui-btn layui-btn-primary">重置</button>
			</div>
		</div>
	</div>
...
var layer = layui.layer; //定义全局变量  方便使用layui的分页条
/* 查询所有员工 */
$("#reny").focus(function(){
  $("#reny").css("background-color","#FFFFCC");
   findAllUser();
});
...
var layer = layui.layer;
	   ...
	   table.render({
	   elem : '#userAll',   //同上
	   url : 'dept/findAllUser',
	   cols : [ [ 
	   {checkbox:true,fixed:true},
	   {field : 'id',title : '用户编号',sort : true}, 
	   {field : 'fullname',title : '用户名称',sort : true},
	   {field : 'phone',title : '联系方式',sort : true}, 
	   {field : 'dept_id',title : '部门编号',sort : true}, 
	   {field : 'dept_name',title : '部门名称',sort : true}, 
		       ] ],
	   page:true
		       });
			/* 监听事件 */
			$("#userAllbtn").click(function(data) {
				var checkStatus = table.checkStatus('userAll'); //test即为基础参数id对应的值
				var length = checkStatus.data.length;
				var id = "";
				var name = "";
				for (var i = 0; i < length; i++) {
				...
				$.ajax({
					type : "post",
					url : "personxc/findByUid",
					data : {"uid":id},
						dataType : "json",
						success : function(d) {
							var xc0 = parseFloat($("#si").children().find("div").eq(0).children().val(d[0].personxc_s1))
							var xc1 = parseFloat($("#si").children().find("div").eq(1).children().val(d[0].personxc_s2))
							var xc2 = parseFloat($("#si").children().find("div").eq(2).children().val(d[0].personxc_s3))
							var xc3 = parseFloat($("#si").children().find("div").eq(3).children().val(d[0].personxc_s4))
							var xc4 = parseFloat($("#si").children().find("div").eq(4).children().val(d[0].personxc_s5))
							var xc5 = parseFloat($("#si").children().find("div").eq(5).children().val(d[0].personxc_s6))
							var xc6 = parseFloat($("#si").children().find("div").eq(6).children().val(d[0].personxc_s7))
							var xc7 = parseFloat($("#si").children().find("div").eq(7).children().val(d[0].personxc_s8))
							var xc8 = parseFloat($("#si").children().find("div").eq(8).children().val(d[0].personxc_9))
							$("#zong").val(d[0].personxc_insurejs); //保险基数
							$("#ylgr").val(d[0].personxc_gryl); //个人养老
							$("#yldw").val(d[0].personxc_dwyl); //单位养老
							$("#ylzong").val(d[0].personxc_ylinsure); //养老保险
							$("#yildw").val(d[0].personxc_dw_medical); //单位医疗
							$("#yilgr").val(d[0].personxc_grmedical); //个人医疗
							$("#yilzong").val(d[0].personxc_ylbx); //医疗保险
							$("#sydw").val(d[0].personxc_dwsy); //单位生育
							$("#syzong").val(d[0].personxc_group); //生育保险
							$("#shiydw").val(d[0].personxc_dwsy); //单位失业
							$("#shiygr").val(d[0].personxc_grsy); //个人失业
							$("#shiyzong").val(d[0].personxc_sybx); //失业保险
							$("#gsdw").val(d[0].personxc_dwgs); //单位工伤
							$("#gszong").val(d[0].personxc_gsbx); //工伤保险
							$("#zfdw").val(d[0].personxc_dwzf); //单位住房
							$("#zfgr").val(d[0].personxc_grzf); //个人住房
							$("#zfzong").val(d[0].personxc_housingfund); //住房公积金
						layer.closeAll();
							$("#kuang").css("display", "none");
						}
					});
		...
		success : function(data) {
			...
			for (var i = 0; i < ary.length; i++) {
				...
				var tr = "<div class='layui-form-item' style='margin-bottom:1px;margin-top:1px;'>" +
					"<label class='layui-form-label'>" + obj.Item_name + "</label><div class='layui-input-inline'><input type='text'  name='personxcS" + n + "' lay-verify='required' placeholder='请输入' autocomplete='off' class='layui-input xinchou'/>" + "</div>";
				$("#si").append(tr); //追加行
			}
		}
	})
...
/* $("body").on("blur",".xinchou",function(){
 alert($(this).val());
}) */
/* 计算 */
function jisuan(){
 var xc0=parseFloat($("#si").children().find("div").eq(0).children().val())
 var xc1=parseFloat($("#si").children().find("div").eq(1).children().val())
 var xc2=parseFloat($("#si").children().find("div").eq(2).children().val())
 var xc3=parseFloat($("#si").children().find("div").eq(3).children().val())
 var xc4=parseFloat($("#si").children().find("div").eq(4).children().val())
 var xc5=parseFloat($("#si").children().find("div").eq(5).children().val())
 var xc6=parseFloat($("#si").children().find("div").eq(6).children().val())
 var xc7=parseFloat($("#si").children().find("div").eq(7).children().val())
 var xc8=parseFloat($("#si").children().find("div").eq(8).children().val())
...
/* 实发工资 */
var sfgz=xc0+xc1+xc2+xc3+xc4-xc5-gr0*0.01*zong-gr1*0.01*zong-gr3*0.01*zong-gr5*0.01*zong
alert("实发工资："+sfgz)
$("#sfgz").val(sfgz);
}
/* 保存数据 */
function personxcInsert() {
	...
	$.ajax({
		url : "saldata/insert?userId=" + $("#adminids").val() + "",
		type : 'post',
		async : true,
